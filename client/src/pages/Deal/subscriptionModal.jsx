...
export default function SubscriptionModal({ onClose }) {
  ...
  const handlePay = async () => {
    ...
    try {
      // Backend now generates the reference and returns authorization_url
      ...
      // Redirect browser to Paystack's hosted payment page.
      // Paystack will redirect back to your backend callback_url after payment,
      // which then redirects to /subscription/verify?status=...
      window.location.href = res.data.authorization_url;

      // No need to setLoading(false) — the page is navigating away
    } catch (err) {
    ...
    <>
      ...
      >
        <div className="modal-anim relative bg-white rounded-2xl border border-gray-100 shadow-2xl w-full max-w-[420px] flex flex-col items-center gap-4 px-6 pt-8 pb-6">
          ...
          {/* Redirecting notice — shown while navigating away */}
          {loading && (
            <p className="w-full text-xs text-blue-600 bg-blue-50 border border-blue-100 rounded-xl px-3 py-2 text-center flex items-center justify-center gap-2">
              <Loader2 size={13} className="spin" />
          ...
          >
            ...
            {loading ? 'Redirecting…' : 'Pay ₦10,000 — Get 10 Points'}
          </button>
