...
//                     }`}>
//                       {order.paymentMethod=== 'online' && order.status === 'confirmed'  ? 'Online order' : order.paymentMethod === 'Pay on Delivery' ? 'Pay on Delivery' : order.paymentMethod === 'pos_cash' ? 'Cash' : order.paymentMethod === 'pos_transfer' ? 'Transfer' : 'POS' }
...
export default function SellerDashboard() {
  ...
  const platformFeeOwed = analytics?.summary?.totalPlatformFee || 0;
  ...
  <StatCard
    icon={AlertTriangle}
    label="Platform Fee Owed"
    value={`₦${platformFeeOwed.toLocaleString()}`}
    sub="To be remitted"
    color="amber"
  />
  <button
    onClick={() => setIsPayModalOpen(true)}
    disabled={platformFeeOwed <= 0}
    className="mt-2 w-full bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold py-1.5 rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-amber-600"
  >
    {platformFeeOwed <= 0 ? 'No Fee Owed' : 'Pay Platform Fee'}
  </button>
</div>
...
<PayPlatformFeeModal
  isOpen={isPayModalOpen}
  onClose={() => setIsPayModalOpen(false)}
  currentBalance={platformFeeOwed}
  onPaymentSuccess={() => {
    // Refresh dashboard stats so the balance and button reflect the payment
    fetchDashboardData();
  }}
/>
