...
import im from "../../assets/AC/priceChecker.jpeg"
...
// Add near the top of the file, alongside useDebouncedValue
let lockCount = 0;
...
export default function PriceCheckers() {
  ...
  // Hero's own "Filter" button — opens a modal mirroring the same filters
  // that already live in the sticky bar below. Purely an extra entry point;
  // nothing about the existing filter bar changes.
  ...
  useLockBodyScroll(filterModalOpen || !!selectedSeller);
  useEffect(() => {
  if (!filterModalOpen) return;
  const onKey = (e) => { if (e.key === 'Escape') setFilterModalOpen(false); };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="relative overflow-hidden">
        <img src={im} alt="" className="absolute inset-0 w-full h-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-r from-black/65 via-black/75 to-black/60" />

        <div className="relative z-10 max-w-3xl mx-auto md:mx-0 px-4 sm:px-8 py-12 sm:py-16">
          ...
          <p className="text-gray-200 text-sm sm:text-base mb-7 max-w-xl">
            Search products across multiple stores and locations, compare prices in one place and make smarter buying decisions.
          </p>

          {/* Search + Filter row */}
          <div className="flex items-center gap-2 max-w-xl mb-3">
            <input
              type="text"
              ...
              className="flex-1 rounded-xl bg-white/95 border border-white/20 px-4 py-3 text-sm text-gray-900 placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-[#8B1E3F]"
            />
            <button
              type="button"
              onClick={() => setFilterModalOpen(true)}
              className="flex items-center gap-1.5 rounded-xl bg-white/15 hover:bg-white/25 text-white border border-white/30 px-4 py-3 text-sm font-medium whitespace-nowrap transition-colors"
            >
              <SlidersHorizontal className="w-4 h-4" />
              Filter
              {activeFilterCount > 0 && (
                <span className="bg-[#8B1E3F] text-white text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center">
                  {activeFilterCount}
                </span>
              )}
            </button>
          ...
          <button
            ...
            className="w-full max-w-xl rounded-xl bg-[#8B1E3F] hover:bg-[#731832] text-white font-semibold py-3 text-sm transition-colors"
          >
            Search
          </button>
        </div>
      ...
      {/* ── Filter modal (opened from the hero's Filter button) ── */}
      {filterModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          <div className="absolute inset-0 bg-black/50" onClick={() => setFilterModalOpen(false)} />
          <div className="relative bg-white w-full max-w-lg rounded-2xl p-6 max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-lg font-semibold text-gray-900">Filter</h3>
              <button onClick={() => setFilterModalOpen(false)} className="text-gray-400 text-2xl leading-none">&times;</button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <input
                ...
                className="rounded-lg border border-gray-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#8B1E3F]"
              />
              <input
                type="number"
                min="0"
            ...
            {priceRangeInvalid && (
              <p className="text-xs text-red-500 mt-2">Min price is higher than max</p>
            )}
            ...
            <div className="flex items-center gap-3 mt-5">
              {activeFilterCount > 0 && (
                <button
                  ...
                  className="text-sm text-red-500 font-medium px-3 py-2.5"
                >
                  Clear all
                </button>
              )}
              <button
                onClick={() => { runSearchNow(); setFilterModalOpen(false); }}
                className="flex-1 rounded-full bg-[#8B1E3F] hover:bg-[#731832] text-white font-semibold py-2.5 text-sm transition-colors"
              >
                Apply
              </button>
            </div>
          ...
          <div className="flex justify-center flex-wrap gap-2 mt-8">
            {Array.from({ length: totalPages }).map((_, i) => (
              <button
                key={i}
                onClick={() => setPage(i + 1)}
                className={`w-9 h-9 rounded-full text-sm font-medium ${page === i + 1 ? 'bg-[#8B1E3F] text-white' : 'bg-white border border-gray-300 text-gray-700'
                  }`}
              >
                {i + 1}
              </button>
...
function FilterSelect({ placeholder, value, onChange, options, capitalize }) {
  ...
  return (
    <select
      ...
      className={`rounded-full border px-3 py-1.5 text-sm bg-white ${value ? 'border-[#8B1E3F] text-[#8B1E3F] font-medium' : 'border-gray-300 text-gray-600'
        }`}
    >
...
// Same normalization as FilterSelect, styled for the modal (block width,
// left-aligned label baked into the placeholder option like the screenshot).
function ModalSelect({ placeholder, value, onChange, options }) {
  ...
  return (
    <select
      value={value}
      onChange={(e) => onChange(e.target.value)}
      className={`w-full rounded-lg border px-3 py-2.5 text-sm bg-white ${value ? 'border-[#8B1E3F] text-[#8B1E3F] font-medium' : 'border-gray-300 text-gray-600'
        }`}
    >
      <option value="">{placeholder}</option>
      {normalized.map((opt) => (
...
function SellerModal({ seller, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative bg-white w-full sm:max-w-2xl sm:rounded-2xl rounded-t-2xl max-h-[60vh] overflow-y-auto p-5">
