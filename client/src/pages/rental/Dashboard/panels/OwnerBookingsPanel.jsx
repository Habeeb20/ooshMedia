...
export default function OwnerBookingsPanel() {
  ...
  if (selected) {
    return <BookingDetailPage booking={selected} role="owner" onBack={() => setSelected(null)} />;
  }

  return (
    <div>
      <h1 className="font-serif text-2xl text-[#221B1D] mb-1">Rental requests</h1>
      <p className="text-[#6B6067] mb-6">Bookings people have made on your items.</p>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 border border-dashed border-[#E7DEE1] rounded-xl">
          <p className="text-[#221B1D] font-medium">No requests yet</p>
          <p className="text-sm text-[#6B6067] mt-1">When someone books your item, it'll show up here.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {bookings.map((b) => (
