...
const PIE_COLORS = ["#C8821F", "#1F9B85", "#D14E37", "#3866A3", "#7A5C99"];
const CHART_HEIGHT = 220; // shared height keeps charts consistent and mobile-friendly
...
export default function Dashboard() {
  ...
  return (
    ...
    >
      ...
      <div className="px-3 py-5 sm:px-4 sm:py-6 md:px-8 md:py-10">
        {/* ---- Top stat grid (always visible) ---- */}
        <section className="grid grid-cols-2 md:grid-cols-4 gap-2.5 sm:gap-3 md:gap-5">
          <StatCard
            icon={<ShoppingBag size={18} />}
            label="Orders placed"
            value={client?.totalOrders ?? 0}
        ...
        {/* ---- Buyer activity chart ---- */}
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-5 mt-6 sm:mt-8">
          <Panel title="Monthly spend" className="md:col-span-2">
            {client?.monthlySpend?.length ? (
              <ResponsiveContainer width="100%" height={CHART_HEIGHT}>
                <AreaChart data={client.monthlySpend} margin={{ top: 10, right: 5, left: 0, bottom: 0 }}>
                  <defs>
                    <linearGradient id="spendFill" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor={COLORS.amber} stopOpacity={0.45} />
                      <stop offset="100%" stopColor={COLORS.amber} stopOpacity={0} />
                  ...
                  />
                  <Tooltip
                    contentStyle={{ background: "#fff", border: `1px solid ${COLORS.panelBorder}`, borderRadius: 8, fontSize: 12 }}
                    labelStyle={{ color: "#101418", fontWeight: 600 }}
                    itemStyle={{ color: COLORS.amber }}
                    formatter={(v) => fmtNaira(v)}
                  />
        ...
        <section className="mt-4 sm:mt-5">
          <Panel title="Monthly spend (bar view)">
            {client?.monthlySpend?.length ? (
              <ResponsiveContainer width="100%" height={CHART_HEIGHT}>
                <BarChart data={client.monthlySpend} margin={{ top: 10, right: 5, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke={COLORS.panelBorder} vertical={false} />
                  <XAxis
                    dataKey="label"
                    stroke={COLORS.textDim}
                  ...
                  <Tooltip
                    ...
                    formatter={(v) => fmtNaira(v)}
                  />
                  <Bar dataKey="total" fill={COLORS.amber} radius={[6, 6, 0, 0]} maxBarSize={40} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <EmptyRow text="No paid orders yet — spend trend will appear once orders come in" />
        ...
        {flags.isSeller && seller && (
          <>
            <SectionHeading icon={<Package size={18} />} title="Seller performance" />
            <section className="grid grid-cols-2 md:grid-cols-4 gap-2.5 sm:gap-3 md:gap-5 mt-4">
              <StatCard icon={<Package size={18} />} label="Products listed" value={seller.totalProducts} accent={COLORS.teal} />
              <StatCard icon={<Wallet size={18} />} label="Revenue earned" value={fmtNaira(seller.totalRevenue)} accent={COLORS.amber} />
              <StatCard icon={<ShoppingBag size={18} />} label="Orders received" value={seller.totalOrders} accent={COLORS.coral} />
              <StatCard
            ...
            <section className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-5 mt-4 sm:mt-5">
              <Panel title="Monthly revenue" className="md:col-span-2">
                {seller.monthlyRevenue?.length ? (
                  <ResponsiveContainer width="100%" height={CHART_HEIGHT}>
                    <BarChart data={seller.monthlyRevenue} margin={{ top: 10, right: 5, left: 0, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke={COLORS.panelBorder} vertical={false} />
                      <XAxis dataKey="label" stroke={COLORS.textDim} fontSize={11} tickLine={false} axisLine={{ stroke: COLORS.panelBorder }} />
                      <YAxis stroke={COLORS.textDim} fontSize={11} tickLine={false} axisLine={false} width={54} tickFormatter={(v) => fmtNaira(v)} />
                      <Tooltip
                        contentStyle={{ background: "#fff", border: `1px solid ${COLORS.panelBorder}`, borderRadius: 8, fontSize: 12 }}
                        labelStyle={{ color: "#101418", fontWeight: 600 }}
                        formatter={(v) => fmtNaira(v)}
                      />
                      <Bar dataKey="revenue" fill={COLORS.teal} radius={[6, 6, 0, 0]} maxBarSize={40} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
                  <EmptyRow text="No revenue yet — this fills in once paid orders come through" />
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5 mt-4 sm:mt-5">
              <Panel title="Top selling products">
                <ul className="divide-y divide-gray-200">
                  {(seller.topProducts || []).map((p) => (
                    <li key={p._id} className="flex items-center justify-between gap-2 py-3">
                      <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
                        <img
                          src={p.images?.[0]?.url || "https://placehold.co/40x40"}
                          alt={p.name}
                          className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg object-cover bg-gray-100 flex-shrink-0"
                        />
                      ...
                      <span className="flex items-center gap-1 text-xs flex-shrink-0" style={{ color: COLORS.amber }}>
                        <Star size={12} fill={COLORS.amber} /> {p.ratings || 0}
                      </span>
                    </li>
                  ))}
                ...
                <ul className="divide-y divide-gray-200">
                  {(seller.lowStockProducts || []).map((p) => (
                    <li key={p._id} className="flex items-center justify-between gap-2 py-3">
                      ...
                      <span className="text-xs px-2 py-1 rounded-full flex-shrink-0" style={{ background: `${COLORS.coral}1A`, color: COLORS.coral }}>
                        {p.stockQuantity} left
                      </span>
                    </li>
                  ))}
          ...
          <>
            ...
            <section className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-5 mt-4 sm:mt-5">
              <Panel title="Monthly earnings" className="md:col-span-2">
                {rider.monthlyDeliveries?.length ? (
                  <ResponsiveContainer width="100%" height={CHART_HEIGHT}>
                    <AreaChart data={rider.monthlyDeliveries} margin={{ top: 10, right: 5, left: 0, bottom: 0 }}>
                      <defs>
                        <linearGradient id="earnFill" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="0%" stopColor={COLORS.teal} stopOpacity={0.45} />
                          <stop offset="100%" stopColor={COLORS.teal} stopOpacity={0} />
            ...
            <section className="mt-4 sm:mt-5">
              <Panel title="Recent deliveries">
                <ul className="divide-y divide-gray-200">
                  {(rider.recentDeliveries || []).map((d) => (
                    <li key={d._id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-2 py-3 text-sm">
                      <div className="flex items-center justify-between sm:justify-start sm:gap-3">
                        <span>{d.orderNumber}</span>
                        <StatusPill status={d.status} />
                      </div>
                      <span className="text-xs sm:text-sm truncate" style={{ color: COLORS.textDim }}>
                        {d.delivery?.address || "—"}
                      </span>
                    </li>
                  ))}
...
function StoreUrlButton({ profile }) {
  ...
  const rootDomain = import.meta.env.VITE_ROOT_DOMAIN || "estores.ng";
  const slug = profile?.businessProfile?.storeSlug || profile?.username || "";
  ...
  return (
    <button
      ...
      title="Copy your store link"
    >
      <span className="font-medium text-gray-700 max-w-[130px] sm:max-w-[160px] truncate">
        {slug}.{rootDomain}
      </span>
      {copied ? (
        <Check size={13} style={{ color: COLORS.teal }} />
...
function Header({ profile, flags, currentTime }) {
  ...
  return (
    <header className="flex flex-col gap-3 p-4 sm:p-5 md:flex-row md:items-center md:justify-between bg-[#8B1E3F]">
      {/* Row 1 on mobile: avatar + greeting + time, kept together so time never gets orphaned by wrapping */}
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
          <img
            src={profile?.profilePicture || "https://placehold.co/56x56"}
            alt={profile?.username}
            className="w-11 h-11 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-2xl object-cover border-2 flex-shrink-0"
            style={{ borderColor: COLORS.amber }}
          />
          <div className="min-w-0">
            <p className="text-rose-200 text-xs sm:text-sm font-medium mb-0.5 sm:mb-1">
              {greetEmoji} {greetText}
            </p>
            <h1 className="text-base sm:text-lg md:text-xl text-white font-semibold truncate">
              Welcome back, {profile?.firstName}
            </h1>
        ...
        <div className="text-right flex-shrink-0 md:order-3">
      ...
      {/* Row 2 on mobile: action pills, full width and wrapping */}
...
function SectionHeading({ icon, title }) {
  return (
    <div className="flex items-center gap-2 mt-8 sm:mt-10 mb-1">
      <span style={{ color: COLORS.amber }}>{icon}</span>
      <h2 className="text-sm sm:text-base md:text-lg font-semibold tracking-tight">{title}</h2>
      <div className="flex-1 h-px ml-2" style={{ background: COLORS.panelBorder }} />
    </div>
  );
}
...
function StatCard({ icon, label, value, accent }) {
  return (
    <div
      className="rounded-2xl p-3 sm:p-4 md:p-5 flex flex-col gap-1.5 sm:gap-2"
      style={{
        background: COLORS.panel,
        borderLeft: `3px solid ${accent}`,
        border: `1px solid ${COLORS.panelBorder}`,
    ...
    >
      ...
      <span className="text-base sm:text-lg md:text-2xl font-semibold truncate">{value}</span>
    </div>
  );
}
...
function Panel({ title, children, className = "" }) {
  return (
    <div
      className={`rounded-2xl p-3 sm:p-4 md:p-5 ${className}`}
      style={{ background: COLORS.panel, border: `1px solid ${COLORS.panelBorder}`, boxShadow: "0 1px 3px rgba(16,20,24,0.05)" }}
    >
      <h3 className="text-xs sm:text-sm font-medium mb-3" style={{ color: COLORS.textDim }}>
        {title}
      </h3>
      {children}
    </div>
  ...
  return (
    <ResponsiveContainer width="100%" height={CHART_HEIGHT}>
      <PieChart>
        ...
        <Tooltip contentStyle={{ background: "#fff", border: `1px solid ${COLORS.panelBorder}`, borderRadius: 8, fontSize: 12 }} />
      </PieChart>
    </ResponsiveContainer>
  );
}
...
function ErrorState({ message }) {
  return (
    <div style={{ background: COLORS.ink, minHeight: "100vh" }} className="flex items-center justify-center text-[#D14E37] text-center px-4">
      {message}
    </div>
  );
}
