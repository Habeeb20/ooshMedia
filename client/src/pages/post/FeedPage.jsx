...
// import EDealsFeed from './EdealsFeed';
// import { useSearchParams, useNavigate } from 'react-router-dom';
// import im from "../../assets/AC/peoplemetting.jpeg"
// const FILTERS = [
...
//   { value: 'edeals', label: 'E Deals', icon: Package },
// ];
...
//     if (type === 'jobs' || type === 'edeals') return;
...
//                     : 'text-black hover:text-white hover:bg-[#8B1E3F]'
//                   }
//                 `}
//               >
//                 <f.icon size={12} />
...
//         ) : postType === 'edeals' ? (
//           // ── E Deals feed rendered inside the same page shell ──
//           <EDealsFeed />
//         ) : (
//           <>
...
import { useState, useEffect, useCallback, useRef } from 'react';
import { Plus, Briefcase, Package, FileText, TrendingUp, RefreshCw, Sparkles, Trophy } from 'lucide-react';
import { feedAPI } from '../../config/api';
import CreatePostModal from './CreatePost';
import PostCard from './PostCard';
import JobFeed from './JobFeed';
...
// Any action that creates/mutates content (opening the create modal,
// deleting a post, etc.) goes through here first. Logged-out users get
// sent to /login instead of the action running; logged-in users pass
// straight through — nothing changes for them.
const requireAuth = (action) => {
  ...
  return (
    <div className="w-full mx-auto px-3  sm:px-4 py-4 pb-28 lg:pb-6">
      ...
      {/* ── Hero ── */}
      <div className="relative  mt-3 overflow-hidden -mx-3 sm:-mx-4 -mt-4 mb-5">
        <img src={im} alt="" className="absolute inset-0 w-full h-full object-cover" />
        <div className="absolute inset-0 bg-black/45" />

        <div className="relative z-10 px-5 sm:px-10 py-14 sm:py-20 max-w-3xl">
          <div className="inline-flex items-center gap-2 bg-amber-100/90 text-amber-900 px-4 py-1.5 rounded-full text-sm font-semibold mb-6">
            <Trophy className="w-4 h-4" />
            Nigeria's Open Market
          </div>

          <h1 className="text-3xl sm:text-5xl font-extrabold text-white mb-5 leading-tight">
            Find Opportunities. Make Deals.<br />
            Get Business Done.
          </h1>
          <p className="text-gray-100 text-sm sm:text-base max-w-xl leading-relaxed">
            Explore contracts, supplies, deals, jobs, and business posts all in one marketplace built to connect people and opportunities.
          </p>
        </div>
      </div>
      ...
      {postType !== 'jobs' && postType !== 'edeals' && (
        <div className="grid grid-cols-3 gap-2.5 mb-4">
          {CREATE_TYPES.map(({ type, label, desc, icon: Icon, bg, border, iconColor, textColor, hoverBg, dot }) => (
            <button
              key={type}
              onClick={() => requireAuth(() => { setCreateType(type); setShowCreate(true); })}
              className={`
                relative flex flex-col items-start gap-2 p-3.5 rounded-2xl border
                ${bg} ${border} ${hoverBg}
                transition-all duration-200 active:scale-95 text-left group
          ...
          <>
            ...
            {!loading && posts.length === 0 && (
              <div className="bg-white rounded-2xl border border-gray-100 p-10 text-center">
                ...
                <p className="text-sm text-gray-400 mb-5">Be the first to post something!</p>
                <button
                  onClick={() => requireAuth(() => { setCreateType('post'); setShowCreate(true); })}
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#8B1E3F] text-white text-sm font-semibold hover:bg-[#7a1835] transition-colors active:scale-95"
                >
                  <Plus size={15} />
                  Create Post
