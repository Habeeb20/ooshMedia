...
// Subcategories in productCategories.js come in two shapes:
// plain strings ("Smartphones") or objects ({ value, label }).
// These two helpers normalize either shape safely.
const getSubLabel = (sub) => (typeof sub === "string" ? sub : sub?.label ?? "");
const getSubValue = (sub) => (typeof sub === "string" ? sub : sub?.value ?? sub?.label ?? "");
...
export default function CategoryDropdown() {
  ...
  const goToSubCategory = (category, sub) => {
    navigate(`/category/${slugify(category.name)}/${slugify(getSubValue(sub))}`);
    setActiveCategory(null);
    setShowModal(false);
  };
  ...
  return (
    <>
      ...
      {/* Modal for "See All" / mobile */}
      {showModal && (
        <div className="fixed inset-0 z-[60] bg-black/60 flex items-center mt-20 justify-center p-4 backdrop-blur-sm">
          <div className="bg-white w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b">
              <div>
                <h2 className="text-lg font-black" style={{ color: appConfig.colors.primary }}>
