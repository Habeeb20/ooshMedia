...
import axios from 'axios';
import { toast } from 'sonner';
import { Loader2, X, ChevronDown, Video } from 'lucide-react';
import CloudinaryUpload from '../../config/CloudinaryUpload';
import { productCategoryList } from '../../categories/productCategories';
...
const ProductVideoUploadForm = ({ onSuccess }) => {
  ...
  return (
    <div className="max-w-xl mx-auto bg-white rounded-2xl border border-slate-200 p-6 space-y-5">
      ...
      {/* Video Upload Container with Active Progress Overlay */}
      <div className="relative">
        <div
          onChangeCapture={() => setUploading(true)}
        ...
        {uploading && (
          <div className="mt-3 flex items-center justify-center gap-3 p-4 bg-[#7B2038]/5 border border-[#7B2038]/20 rounded-xl">
            <Loader2 className="w-5 h-5 text-[#7B2038] animate-spin" />
            <span className="text-sm font-medium text-[#7B2038]">
        ...
        {videoData && !uploading && (
          <div className="mt-3 flex items-center justify-between p-3 bg-emerald-50 border border-emerald-200 rounded-xl text-xs font-medium text-emerald-800">
            <div className="flex items-center gap-2">
              <Video className="w-4 h-4 text-emerald-600" />
              <span>Video attached ready for posting</span>
            </div>
            <button
              type="button"
      ...
      <button
        onClick={handleSubmit}
        disabled={submitting || uploading || !videoData}
        className="w-full flex items-center justify-center gap-2 bg-[#7B2038] text-white font-medium py-2.5 rounded-xl hover:bg-[#611829] transition disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
        {submitting ? 'Posting...' : 'Post video'}
