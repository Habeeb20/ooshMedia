...
export default function JobLocationMap({ lga, state, address }) {
  const { location: contextUserLocation } = useUserLocation();
  ...
  // Get the user's CURRENT position fresh, at the moment the map is viewed
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        console.warn('Live location fetch failed, falling back to cached location', err);
  ...
  // Prefer the fresh live position; fall back to context if geolocation
  // was denied/unavailable.
  const userLocation = liveUserLocation || contextUserLocation;

  const { jobCoords, distanceKm, driveMinutes, distanceLoading } = useJobDistance(userLocation, lga, state);
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      >
        ...
        {/* Blue Line between user and job, with a distance/time label at its midpoint */}
        {userLocation && jobCoords && (
          <>
            <Polyline
              path={[
            ...
            {distanceKm != null && (
              ...
              >
                <div className="text-xs font-semibold text-gray-700 whitespace-nowrap px-1">
                  {distanceKm < 1 ? `${Math.round(distanceKm * 1000)}m` : `${distanceKm.toFixed(1)} km`} · ~{driveMinutes} min
                </div>
              </InfoWindow>
            )}
          </>
