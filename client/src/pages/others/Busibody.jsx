...
//   Sparkles,
// } from "lucide-react";
// import im from "../../assets/beauty.jpeg"
// import im2 from "../../assets/beauty1.jpeg"
// import im1 from "../../assets/beauty2.jpeg"
...
import im2 from "../../assets/AC/beauty1.jpeg";
import im1 from "../../assets/AC/beauty2.jpeg";
import im from "../../assets/AC/beauty.jpeg";
...
/* ────────────────────────────────────────────────────────────────────────
   Small helper: renders an <img>, swaps to a muted placeholder on error
   so a broken import path is visible instead of a silent blank patch.
   ──────────────────────────────────────────────────────────────────────── */
function ImgWithFallback({ src, alt, className }) {
  const [failed, setFailed] = useState(false);
  if (failed) return <div className={`${className} bg-[#241811]`} />;
  return (
    <img
      src={src}
      alt={alt}
      ...
      loading="eager"
      onError={() => {
        console.error(`[Hero] Failed to load image: ${alt} → check the import path.`, src);
        setFailed(true);
      }}
    />
  );
...
/* ────────────────────────────────────────────────────────────────────────
   HeroBackgroundImages — the three hero photos tiled full-bleed behind the
   hero text. Tinted with the same brand gradient (via blend mode) and
   darkened with an overlay for legibility. Static, no animation.
   ──────────────────────────────────────────────────────────────────────── */
function HeroBackgroundImages() {
  const images = [
    { src: im, alt: "Beauty styling session" },
    { src: im2, alt: "Nail and spa detail" },
    { src: im1, alt: "Skincare product close-up" },
  ];

  return (
    <div className="absolute inset-0">
      {/* the three photos, tiled edge-to-edge to fill the hero */}
      <div className="absolute inset-0 flex">
        {images.map((img) => (
          <ImgWithFallback
            ...
            src={img.src}
            alt={img.alt}
            className="flex-1 h-full object-cover mix-blend-luminosity opacity-60"
          />
        ))}
      </div>

      {/* tints the photos with the brand gradient */}
      <div
      ...
      {/* dark overlay for text legibility, stronger on the left where copy sits */}
      <div className="absolute inset-0 bg-gradient-to-r from-[#0D0906]/90 via-[#0D0906]/70 to-[#0D0906]/50" />
      <div className="absolute inset-0 bg-[#0D0906]/25" />
    </div>
  );
}
    ...
    <div className="min-h-screen bg-[#F8F4EE] text-[#1C140F]" style={{ fontFamily: "'Manrope', sans-serif" }}>
      ...
      <section className="relative overflow-hidden">
        ...
        >
          ...
          <div className="relative z-20 max-w-7xl mx-auto w-full py-20 sm:py-28 lg:py-32">
            <motion.div
              initial={{ opacity: 0, y: 24 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.7 }}
            ...
            >
              ...
              <p className="text-white/70 text-sm sm:text-base max-w-md mx-auto lg:mx-0 mb-8">
                Where talent, convenience, and craft come together in your community — book trusted
                beauty professionals and shop artisan essentials in one place.
              </p>
