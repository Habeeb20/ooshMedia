...
function PostCard({ post: initialPost, onFinalized }) {
  ...
  // Toggle between active <-> closed. Draft posts are left untouched here —
  // they should be published before they can be finalized/reopened.
  const handleToggleStatus = async () => {
    if (finalizing) return;
    const closing = post.status === 'active';
    const confirmMsg = closing
      ? 'Mark this post as finalized/closed?'
      : 'Reopen this post and make it active again?';
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setPost(p => ({ ...p, status: data.data.status })); // trust server's new status
        onFinalized?.(post._id);
      }
    } finally { setFinalizing(false); }
  };
        ...
        {expanded && (
          <div className="border-t border-gray-100 px-4 sm:px-5 pb-5">
            <div className="pt-4 space-y-4">
              ...
              {['active', 'closed'].includes(post.status) && (
                <button
                  onClick={handleToggleStatus}
                  disabled={finalizing}
                  className={`flex items-center gap-2 text-xs font-bold px-4 py-2 rounded-xl border transition-all disabled:opacity-50 ${
                    post.status === 'active'
                  ...
                  {finalizing
                    ? <span className={`w-3 h-3 border-2 rounded-full animate-spin ${
                        post.status === 'active'
                          ...
                          : 'border-amber-600/30 border-t-amber-600'
                      }`} />
                    : post.status === 'active' ? <CheckIcon /> : <ReopenIcon />
                  }
                  {post.status === 'active' ? 'Mark as Finalized' : 'Reopen Post'}
                </button>
              )}
