...
import PropertyDetailsModal from './ProperDetailsModal';
...
import { fetchCars,fetchProperties, effectiveCarPrice, effectivePropertyPrice } from './CarPropertiesApi';
import { fetchProducts, effectivePrice } from './pricecheckerApi';


export default function ResultsView({ category, initialFilters, onLogoClick, onCategoryChange }) {
  const [search, setSearch] = useState(initialFilters.search || '');
  const [filters, setFilters] = useState(() => buildInitialFilters(category, initialFilters));
  const [page, setPage] = useState(1);
  const [result, setResult] = useState(null);
  const [status, setStatus] = useState('loading'); // loading | ready | error
  const [errorMsg, setErrorMsg] = useState('');
  ...
  // reset local state whenever the category itself changes (e.g. switching tabs mid-results)
  useEffect(() => {
    setFilters(buildInitialFilters(category, initialFilters));
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        let data;
        if (category === 'cars') {
          data = await fetchCars({ search, ...filters, page, limit: 20 });
          if (!cancelled && data.makes) setCarMakes(data.makes);
        } else if (category === 'properties') {
          data = await fetchProperties({ search, ...filters, page, limit: 20 });
        } else {
          const raw = await fetchProducts({ search, ...filters, page, limit: 20 });
          data = { items: raw.products || [], page: raw.page, totalPages: raw.totalPages };
        }
        if (cancelled) return;
        setResult(data);
        setStatus('ready');
  ...
  const priceOf = category === 'cars' ? effectiveCarPrice : category === 'properties' ? effectivePropertyPrice : effectivePrice;

  const bestPrice = useMemo(() => {
    if (!items.length) return null;
    if (category === 'products') {
      const eligible = items.filter((p) => p.status !== 'out_of_stock' && p.stockQuantity !== 0);
      return eligible.length ? Math.min(...eligible.map(effectivePrice)) : null;
    }
    return Math.min(...items.map(priceOf));
  }, [items, category, priceOf]);

  const CardComponent = category === 'cars' ? CarCard : category === 'properties' ? PropertyCard : ProductCard;
    ...
    <div className="min-h-screen bg-[#f6f4f9]">
      <div className="border-b border-[#ece7f0] bg-white">
        <div className="mx-auto max-w-7xl px-5 py-8 text-center sm:px-8">
          <div className="mb-5 flex justify-center">
            <CategoryTabs value={category} onChange={handleCategorySwitch} />
          </div>

          <h1 className="font-serif text-2xl font-bold text-[#1d1922] sm:text-3xl">
            {category === 'cars' ? 'Compare Car Prices' : category === 'properties' ? 'Compare Land & Property Listings' : 'Compare Prices Across Multiple Stores'}
          </h1>
          <p className="mt-1.5 text-[13px] text-[#8a8291] sm:text-sm">
            Fast, simple, and reliable analytical pricing for savvy consumers.
          </p>
      ...
      <div className="mx-auto flex max-w-7xl gap-8 px-5 py-8 sm:px-8">
        <aside className="hidden w-[260px] shrink-0 lg:block">
          <div className="sticky top-6 rounded-2xl border border-[#ece7f0] bg-white p-6">
            {category === 'cars' ? (
              <CarFilterPanel filters={filters} onChange={handleFilterChange} onClear={handleFilterChange} makes={carMakes} />
            ) : category === 'properties' ? (
              <PropertyFilterPanel filters={filters} onChange={handleFilterChange} onClear={handleFilterChange} />
            ) : (
              <FiltersPanel filters={filters} onChange={handleFilterChange} onClear={handleFilterChange} />
            )}
          </div>
        </aside>
              ...
              <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3">
                {items.map((item) => (
                  <CardComponent
                    ...
                    {...(category === 'cars' ? { car: item } : category === 'properties' ? { item } : { product: item })}
                    isBestPrice={bestPrice !== null && priceOf(item) === bestPrice && item.status !== 'out_of_stock'}
      ...
      {category === 'cars' && <CarDetailsModal car={activeItem} onClose={() => setActiveItem(null)} />}
      {category === 'properties' && <PropertyDetailsModal item={activeItem} onClose={() => setActiveItem(null)} />}
      {category === 'products' && <ProductDetailsModal product={activeItem} onClose={() => setActiveItem(null)} />}
    </div>
  );
}

function buildInitialFilters(category, initialFilters) {
  if (category === 'cars') {
    return {
      make: initialFilters.make || '', model: initialFilters.model || '', year: initialFilters.year || '',
      transmission: initialFilters.transmission || '', fuelType: initialFilters.fuelType || '',
      bodyType: initialFilters.bodyType || '', condition: initialFilters.condition || '',
      state: initialFilters.state || '', minPrice: initialFilters.minPrice || '', maxPrice: initialFilters.maxPrice || '',
      sort: initialFilters.sort || 'newest',
    };
  }
  if (category === 'properties') {
    return {
      listingType: initialFilters.listingType || '', landType: initialFilters.landType || '', kind: initialFilters.kind || '',
      state: initialFilters.state || '', minPrice: initialFilters.minPrice || '', maxPrice: initialFilters.maxPrice || '',
      sort: initialFilters.sort || 'newest',
    };
  }
  return {
    sellerType: initialFilters.sellerType || '', state: initialFilters.state || '', category: initialFilters.category || '',
    minPrice: initialFilters.minPrice || '', maxPrice: initialFilters.maxPrice || '', sort: initialFilters.sort || 'newest',
  };
}
