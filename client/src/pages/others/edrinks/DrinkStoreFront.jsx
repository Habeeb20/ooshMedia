...
import {
  ...
  BadgeCheck, Plus, Minus, Loader2, LayoutGrid, List,
} from "lucide-react";
...
import CartSummaryModal from "../food/CartSummaryModal";
...
function ProductGridCard({ product, onOpenModal }) {
  const primaryImage = product?.images?.find((img) => img.isPrimary)?.url || product?.images?.[0]?.url || "/placeholder-food.png";
  const hasDiscount = product?.salePrice && product.salePrice < product.price;
  const displayPrice = hasDiscount ? product.salePrice : product.price;
  const qtyInCart = getCartQty(product._id);
  ...
  return (
    <div className="flex flex-col">
      <div className="relative">
        <img src={primaryImage} alt={product.name} className="w-full aspect-square rounded-2xl object-cover" />
        <button
          onClick={(e) => { e.stopPropagation(); onOpenModal(product); }}
          className="absolute -bottom-2 right-2 min-w-8 h-8 px-2 rounded-full bg-emerald-600 text-white shadow-md flex items-center justify-center gap-1 hover:bg-emerald-700 hover:scale-105 active:scale-95 transition-all text-xs font-bold"
          aria-label="Add to cart"
      ...
      <h4 className="mt-3 font-bold text-gray-900 text-sm line-clamp-1">{product.name}</h4>
      <p className="mt-0.5 text-xs text-gray-500 line-clamp-2">{product.description}</p>
      <div className="mt-2 flex items-center gap-2">
        {hasDiscount && (
          <span className="text-xs text-gray-400 line-through">₦{product.price.toLocaleString()}</span>
        )}
        <span className="text-xs font-semibold text-gray-900">From ₦{displayPrice?.toLocaleString()}</span>
      </div>
    </div>
  );
}
    ...
    <div className="mt-10 border-t border-gray-100 pt-8">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleWriteReview} className="text-sm font-semibold text-rose-900 border border-rose-900 rounded-full px-4 py-2 hover:bg-emerald-50">
            Write a Review
          </button>
        <button
  onClick={() => navigate('/food#seller-reviews')}
...
export default function DrinkStorefront() {
...
useEffect(() => {
  const init = async () => {
    ...
    try {
      const { data } = await axios.get(`${API}/api/auth/storefront/${slug}`);
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      const sellerProducts = allProducts.filter((p) => {
        ...
        const isFoodOrGrocery = category.includes("drinks") || category.includes("drinks");
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 mt-6 sm:mt-10">
      <div className="flex items-center gap-2 text-xs font-semibold text-gray-400 tracking-wide">
        <span className="cursor-pointer" onClick={() => navigate("/food")}>HOME</span>
...
<div className="mt-2 flex justify-end">
  <button
    ...
    className="w-9 h-9 rounded-full border border-gray-200 flex items-center justify-center hover:bg-gray-50 active:scale-95 transition-all"
    aria-label={viewMode === "list" ? "Switch to grid view" : "Switch to list view"}
  >
    {viewMode === "list" ? (
      <LayoutGrid className="w-3 h-4 text-gray-700" />
    ) : (
  ...
  <div key={groupName} className="mt-8">
    ...
    {viewMode === "grid" ? (
      <div className="mt-4 grid grid-cols-2 gap-3">
        ...
        {items.map((product) => (
          <ProductGridCard key={product._id} product={product} onOpenModal={setAddModalProduct} />
        ))}
      </div>
    ) : (
      <div>
