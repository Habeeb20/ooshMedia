...
//                     </span>
//                   )}
//                   {order.fulfillmentType === 'pickup' && order.pickup?.code && order.status !== 'delivered' && (
//   <span className="text-xs bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-full">
      ...
      <div className="flex gap-4 mb-5 border-b border-gray-200">
        {['orders', 'analytics'].map(t => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`pb-2 capitalize text-sm font-semibold border-b-2 transition-all ${tab === t ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-400'
              }`}
          >
            {t}
          </button>
                ...
                <div className="text-right flex flex-col items-end gap-1.5 shrink-0">
                  ...
                  {order.status === 'delivered' && (
                    <button
                      ...
                      className="text-xs font-semibold text-emerald-600 border border-emerald-600 rounded-full px-2.5 py-0.5 hover:bg-emerald-50"
                    >
                      Write Review
                    </button>
                  )}
