...
import { useEffect, useState } from 'react';
import { Loader2, Package, Copy, Check, ArrowLeft, ChevronRight, KeyRound } from 'lucide-react';
import DeliveryTrackingMap from '../DeliverytrackingMap';
...
export default function SellerOrdersDashboard() {
  ...
  const handleVerifyPickupCode = async () => {
    if (pickupCodeInput.length !== 4) {
      setPickupError('Enter the 4-digit code');
      return;
    }
    setPickupError('');
    setVerifyingPickup(true);
    try {
      const { data } = await api.post(`/api/orders/${order._id}/verify-delivery`, {
        code: pickupCodeInput,
      });
      setOrder(data.order);
      setShowPickupInput(false);
      setPickupCodeInput('');
      showToast('Pickup confirmed — order marked as delivered!');
    } catch (err) {
      setPickupError(err.response?.data?.message || 'Invalid code, please try again');
    } finally {
  ...
  const pickupAlreadyVerified = order.pickup?.isCodeVerified || order.status === 'delivered';
      ...
      {isDelivery && deliveryReq ? (
        ...
        <div className="max-w-lg mx-auto px-4 py-8">
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            {pickupAlreadyVerified ? (
              <div className="mt-4 flex items-center gap-2 bg-green-50 text-green-700 rounded-xl px-4 py-3 text-sm font-medium">
                <Check size={16} />
                Pickup confirmed
              </div>
            ) : !showPickupInput ? (
              <button
                ...
                className="mt-4 w-full flex items-center justify-center gap-2 bg-rose-900 text-white font-semibold py-3 rounded-xl active:scale-[0.99] transition-transform"
              >
                <KeyRound size={16} />
                Enter Pickup Code
              </button>
              ...
              <div className="mt-4 space-y-2">
                <p className="text-xs text-gray-500">
                  Ask the buyer for their 4-digit pickup code to confirm this order was collected.
                </p>
                <input
                  type="text"
                  inputMode="numeric"
                  ...
                  placeholder="0000"
                  className="w-full text-center text-2xl font-bold tracking-[0.5em] border border-gray-200 rounded-xl py-3 focus:outline-none focus:border-rose-900"
                  autoFocus
                />
                {pickupError && (
                  <p className="text-red-600 text-xs text-center">{pickupError}</p>
                )}
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    disabled={verifyingPickup || pickupCodeInput.length !== 4}
                    className="flex-1 py-3 rounded-xl bg-rose-900 text-white font-semibold disabled:opacity-50 flex items-center justify-center gap-2"
                  >
                    {verifyingPickup ? <Loader2 size={16} className="animate-spin" /> : null}
                    {verifyingPickup ? 'Verifying...' : 'Submit'}
                  </button>
                </div>
              </div>
            )}
